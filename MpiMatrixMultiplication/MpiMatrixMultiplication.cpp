...
int main(int argc, char** argv)
{
	...
	if (taskId == 0)
	{
		...
		if(!TryWriteToFile(file, GetVectorById(taskId, vectorSize, size)))
		{
			MpiErrorExit("TryWriteToFile: return false");
		}
		if (!IsFileClosedSuccessfully(file)) {
			MpiErrorExit("IsFileClosedSuccessfully: return false");
		}
	}
